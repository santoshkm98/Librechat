...
const handleSummarizeContract = useCallback(async () => {
  try {
    const latestFile = Array.from(files.values()).slice(-1)[0];
    ...
    const fileId = latestFile?.file_id;
...
summarize the document
    ...
    submitMessage({
      text: '❌ Failed to summarize contract',
    });
  }
}, [files, submitMessage]);
  ...
  disabled={filesLoading || isSubmitting || disableInputs || files.size === 0}
